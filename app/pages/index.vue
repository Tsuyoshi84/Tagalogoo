...
<template>
	...
	>
		<section
			class="w-full max-w-xl rounded-3xl border border-slate-700 bg-slate-950/80 p-8 shadow-xl backdrop-blur"
		>
			<header class="flex flex-col gap-2">
				<p class="text-sm uppercase tracking-widest text-emerald-300">
					Dashboard
				</p>
			...
			<div class="mt-8 flex flex-wrap items-center gap-3">
				<button
					type="button"
					class="inline-flex items-center justify-center gap-2 rounded-lg border border-slate-500 px-4 py-2.5 font-medium text-slate-100 transition hover:bg-slate-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white disabled:cursor-not-allowed disabled:opacity-60"
					:disabled="isPending"
					@click="handleSignOut"
				>
					Sign out
				...
				<NuxtLink
					class="inline-flex items-center justify-center gap-2 rounded-lg bg-emerald-500 px-4 py-2.5 text-sm font-semibold text-emerald-950 shadow hover:bg-emerald-400 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-300"
					to="/login"
				>
					Switch account
				</NuxtLink>
